...
| 基础设施（Docker/CI）| ~40% | GitHub Actions 配置、docker-compose 整理、npm 工具链统一 |
...
目前 WorkPal 的单测（`auth_svc`、`message_svc`、`conversation_svc`、`presence_svc`、文件服务、搜索服务、Hub 并发）以人工事后补充和修正为主。普通 `go test ./...` 可作为本地基础验证，`go test -race ./...` 建议在 Linux CI 或本机可用 race detector 的环境中运行。

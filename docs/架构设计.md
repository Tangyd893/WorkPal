...
每个领域服务在 `backend/migrations/<service>/` 下配有 golang-migrate 格式的版本化 SQL 迁移。
...
- 每个迁移包含 `up.sql`（前滚）和 `down.sql`（回滚）
- 开发阶段可以使用 GORM AutoMigrate 快速建表
- 正式部署和 CI 环境使用版本化迁移，确保 schema 变更有迹可循
- 迁移通过 Makefile 统一管理：`make migrate-up SERVICE=<name>`
...
- 实时链路与异步链路都有较高学习价值
- 版本化数据库迁移体系已建立
- Repository 接口模式支持单元测试 mock
- 本地运行成本仍然可控

## 12. 当前架构仍然保留的不足

- 还没有独立配置中心
- 还没有分布式链路追踪（OpenTelemetry）

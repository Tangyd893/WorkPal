...
当前实现里，聊天、用户、文件、搜索等核心数据由后端支撑；任务和日程仍主要是前端本地 Demo 状态，适合作为后续后端化练习。
...
项目采用单体模块化结构，而不是一开始就拆微服务。这样更适合学习：业务边界清楚，部署成本低，后续也能按接口逐步替换为更重的生产方案。
...
### 4.4 本地 Demo 状态与后端真实数据并存

当前前端有两类数据：

- 后端真实数据：登录、用户、部门、会话、消息、搜索、文件。
- 前端本地 Demo：任务、日程、部分种子知识卡片。

这个设计让产品体验更完整，但学习时要分清哪些数据会持久化。后续可以把任务和日程模块后端化，作为扩展练习。
...
当前任务和日程主要是前端本地状态。可以新增：

- `task` 模块：任务 CRUD、状态流转、负责人、参与人。
- `schedule` 模块：日程 CRUD、参与人、会议室、提醒。
- 前端把本地状态替换为真实 API。
